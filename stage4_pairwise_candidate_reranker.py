...
def args_parse():
 ...
 p.add_argument("--hard-k",type=int,default=10); p.add_argument("--hard-pool",type=int,default=100)
...
def fold_ids(n,seed):
 ...
 return ids
@torch.no_grad()
def make_pair_cache(data,teacher_spec,device,batch,hard_pool,k):
 ...
 for idx,probes in groups:
  for st in range(0,len(idx),batch):
   ...
   ht,hs,av,y=[x.to(device) for x in (ht,hs,av,y)]; z=mask_logits(probes,ht,hs,av); top=torch.topk(z[0],hard_pool,1).indices
   jj=torch.stack([top[r][top[r]!=y[r]][:k] for r in range(len(y))]); base_y=z[0].gather(1,y[:,None]); base_j=z[0].gather(1,jj)
   for q in range(s):
...
def add_rank(acc,name,scores,cand,y):
 for i in range(len(y)):
  ...
  if not len(pos):acc[name].append(0);continue
  acc[name].append(1+int((scores[i]>scores[i,pos[0]]).sum()))
@torch.no_grad()
def evaluate(data,probes,models,item,args,device,sources,user_choice,thresholds):
 ...
 for ht,hs,av,y,_ in DataLoader(ProbeDataset(data),batch_size=args.score_batch_size):
  ...
  singles=ss[1:1+len(sources)];add_rank(acc,"oracle",torch.maximum(ss[0],singles.max(0).values).cpu().numpy(),cc,yy)
  ...
  off+=b
 def met(r):
  r=np.asarray(r);c=r>0;hit=(r>0)&(r<=10);return {"coverage":float(c.mean()),"positive_users":int(c.sum()),"Recall@10":float(hit.mean()),"NDCG@10":float(np.where(hit,1/np.log2(r+1),0).mean()),"MRR@M":float(np.where(c,1/r,0).mean()),"conditional_Recall@10":float((r[c]<=10).mean()),"conditional_NDCG@10":float(np.where(r[c]<=10,1/np.log2(r[c]+1),0).mean())}
 return {k:met(v) for k,v in acc.items()}
def main():
 args=args_parse();seed_all(args.seed);root=Path(__file__).resolve().parent;device=torch.device(args.device);out=Path(args.output_dir).resolve() if args.output_dir else root/"save/pairwise_reranker"/args.target_dom;out.mkdir(parents=True,exist_ok=True)
 ...
 probes=folds_load(root,args.target_dom,base,sources,device);train_c=make_pair_cache(splits["train"],(fold_ids(len(splits["train"].users),args.seed),probes),device,args.score_batch_size,args.hard_pool,args.hard_k);val_c=make_pair_cache(splits["valid"],probes,device,args.score_batch_size,args.hard_pool,args.hard_k)
 ...
 scale=float(np.std(train_c["pair"]));thresholds=np.linspace(-scale,scale,11).tolist();uv=read_user_choice(root/"save/stage2_utility_router"/args.target_dom/"predictions_valid.csv",splits["valid"].users,sources);vr=evaluate(splits["valid"],probes,models,item,args,device,sources,uv,thresholds)
 chosen={k:max(thresholds,key=lambda t:vr[f"{k}:{t:.6g}"]["NDCG@10"]) for k in models};ut=read_user_choice(root/"save/stage2_utility_router"/args.target_dom/"predictions_test.csv",splits["test"].users,sources);tr=evaluate(splits["test"],probes,models,item,args,device,sources,ut,sorted(set(chosen.values())));fixed=max(range(len(sources)),key=lambda d:vr[f"fixed{d}"]["NDCG@10"])
 report={"protocol":{"hard_negatives":"OOF fold-specific target Top-100, K=10","candidate_pool":"OOF ensemble target Top-5000","threshold":"validation NDCG@10","test":"frozen"},"args":vars(args),"sources":sources,"valid":vr,"chosen_thresholds":chosen,"fixed_source":sources[fixed],"test":tr};save_json(out/"pairwise_reranker_report.json",report);print(json.dumps({"chosen":chosen,"fixed":sources[fixed],"test":tr},indent=2));print(f"[REPORT] {out/'pairwise_reranker_report.json'}")
