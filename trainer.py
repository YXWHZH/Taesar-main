...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.saved_model_file = config["full_ckpt"]
    ...
    def _build_optimizer(self, **kwargs):
        ...
        if self.config["reg_weight"] and weight_decay and weight_decay * self.config["reg_weight"] > 0:
            self.logger.warning("The parameters [weight_decay] and [reg_weight] are specified simultaneously, which may lead to double regularization.")
    ...
    def resume_checkpoint(self, resume_file):
        ...
        if checkpoint["config"]["model"].lower() != self.config["model"].lower():
            self.logger.warning(
                "Architecture configuration given in config file is different from that of checkpoint. This may yield an exception while state_dict is being loaded."
            )
    ...
    def pretrain(
        ...
        for epoch_idx in range(self.start_epoch, 20):
            # train
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_end_time = time()
                valid_score_output = (set_color("epoch %d evaluating", "green") + " [" + set_color("time", "blue") + ": %.2fs, " + set_color("valid_score", "blue") + ": %f]") % (
                    epoch_idx,
    ...
    def evaluate(
        ...
        if test:
            self.wandblogger.log_eval_metrics(result, head="test")
        return result
