...
# -*- coding: utf-8 -*-
"""
audit_multisource_utility.py  (v2: local-ID fix)
===============================================
...
def align_for_target(
    ...
    if not users:
        if len(target_encoded.target_inner_ids):
            valid_ids = target_encoded.target_inner_ids[
                target_encoded.target_inner_ids >= 0
            ]
            if len(valid_ids):
                observed = (
                    int(valid_ids.min()),
            ...
            observed = None
        raise RuntimeError(
            "No aligned users remained after alignment. "
            f"Expected LOCAL target range=[{target_begin_id}, {target_end_id}], "
            f"observed target-id min/max={observed}, stats={stats}"
        )
...
def main() -> int:
    ...
    for d in domains:
        ...
        if d == args.target_dom:
            # IMPORTANT:
            # For train_type=sim / stage=tun, each domain-specific RecBole
            # SequentialDataset owns an independent LOCAL item-id space:
            #   padding = 0
            #   real items = 1 .. dataset.item_num - 1
            #
            # model.domain_ranges[d] belongs to Taesar's mixed/global id space and
            # must NOT be used here for dom2/dom3/dom4. dom1 happened to work only
            # because the first global domain also starts at id 1.
            begin, end = 1, int(dataset.item_num) - 1
            target_range = (begin, end)
            target_hidden_size = int(model.hidden_size)

            if model.item_embedding.weight.shape[0] != int(dataset.item_num):
                raise RuntimeError(
                    "Local-ID assumption violated: "
                    f"embedding_rows={model.item_embedding.weight.shape[0]} "
                    f"dataset.item_num={dataset.item_num}"
                )
            ...
            expected_classes = end - begin + 1
            if target_item_embeddings_cpu.shape[0] != expected_classes:
                raise RuntimeError(
                    "Target classifier size mismatch: "
                    f"got={target_item_embeddings_cpu.shape[0]} "
                    f"expected={expected_classes}"
                )

            global_range = model.domain_ranges.get(d, None)
            print(
                f"Target LOCAL item range : [{begin}, {end}] "
                f"classes={expected_classes}"
            )
            print(
                f"Taesar GLOBAL range     : {global_range} "
                "(diagnostic only; not used by this audit)"
            )
