...
from utils import check_path, log_metrics_table, set_seed

# debugpy.listen(("0.0.0.0", 3187))
...
def main_worker():
    ...
    parser.add_argument("--eval_batch_size", type=int, default=1024, help="number of batch_size")
    ...
    with open(f"data/{args.data_name}/{args.data_name}_list_dataset_{args.strd_ym}.pkl", "rb") as fp:
    ...
    len_voca = []
    for i in columns:
        output_voca_path = f"data/{args.data_name}/{args.data_name}_voca_{i}_{args.strd_ym}.ep"
    ...
    for epoch in range(args.epochs):
        ...
        if int(os.environ.get("LOCAL_RANK", 0)) == 0:
            test_dataset = CausalDataset(args, user_seq, data_type="test")
            test_sampler = SequentialSampler(test_dataset)
            test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.eval_batch_size, num_workers=30, persistent_workers=True)
            total_result = trainer.pretrain(epoch, test_dataloader, train=False)
            # print(f"DEBUG_OUTPUT: Epoch {epoch} Evaluation Raw Result: {total_result}")
            log_metrics_table("tst", total_result)

            print("Previous ndcg@10: ", evaluation_performance, "|| Current ndcg@10: ", total_result[0]["ndcg@10"])
            if evaluation_performance <= total_result[0]["ndcg@10"]:
                print("---------------Evaluation Result-------------------")
                print("                  Update !!!                       ")
                evaluation_performance = total_result[0]["ndcg@10"]
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        ...
        #     # print(result)
        #     log_metrics_table("tst", result)
        # ???
        log_metrics_table("tst", total_result)

        with open(args.log_file, "a") as f:
            f.write(args_str + "\n")
            f.write(str(total_result[0]) + "\n")
