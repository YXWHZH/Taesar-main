...
class TorchVocab(object):
    ...
    def __init__(self, counter, max_size=None, min_freq=5, specials=["<pad>", "<oov>"], vectors=None, unk_init=None, vectors_cache=None):
        ...
        # words_and_frequencies = sorted(counter.items(), key=lambda tup: tup[0])  # 0
        # words_and_frequencies.sort(key=lambda tup: tup[1], reverse=True)  # 1
...
class WordVocab(Vocab):
    def __init__(self, texts, max_size=None, min_freq=1):  # texts-->list
        ...
        print("Building Vocab")
        counter = Counter()
        for line in tqdm.tqdm(texts.values):
