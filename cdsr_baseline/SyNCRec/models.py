...
from modules import Encoder, EncoderNoFFN, Intermediate, LayerNorm, NoisyGating, PatitionedGating
...
class CausalModel(nn.Module):
    ...
    def pretrain_seq(self, item_input, item_pos, item_neg, test_neg, item_answer, type_input, type_pos):
        ...
        for domain_index in domain_list:
            ...
            # encoder
            if self.args.expert_layer == "ffn":
                encoded_layers = self.att_layer(sequence_emb_single, extended_attention_mask, output_all_encoded_layers=True)
                sequence_enc_single = encoded_layers[-1]  # [B L H]
                gate_value = self.gate_[domain_index - len(domain_list)](
                    sequence_emb_single.view([sequence_emb_single.size(0), -1]), cross_yn="n", basic_gating=self.basic_gating
                ).unsqueeze(1)  # B x 1 x expert_num
                gate_value_list.append(torch.mean(gate_value, dim=0)[0])

                fea_1_single = torch.stack(
                    [self.expert_[i](sequence_enc_single).view(sequence_enc_single.size(0), -1) for i in range(single_expert_num)], dim=1
                )  # expert_num B T*D
                fea_2_single = torch.stack(
                    [self.expert_[i + single_expert_num](sequence_enc_single).view(sequence_enc_single.size(0), -1) for i in range(cross_expert_num)], dim=1
                )  # expert_num B T*D
                ...
                fea_1_single = torch.stack(
                    [
                        self.expert_[i](sequence_emb_single, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_emb_single.size(0), -1)
                        for i in range(single_expert_num)
                    ],
                    dim=1,
                )  # expert_num B T*D
                fea_2_single = torch.stack(
                    [
                        self.expert_[i + single_expert_num](sequence_emb_single, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_emb_single.size(0), -1)
                        for i in range(cross_expert_num)
                    ],
                    dim=1,
                )  # expert_num B T*D
                if self.args.single_detach == "y":
            ...
            # single domain loss
            single_domain_loss = self.cross_entropy_single(
                sequence_output_single_tmp, item_pos * (type_pos == domain_index), item_neg * (type_pos == domain_index), type_input * (type_pos == domain_index)
            )
        ...
        if self.args.lrb == "y":
            ...
            if self.args.expert_layer == "ffn":
                encoded_layers = self.att_layer(sequence_emb_cross, extended_attention_mask, output_all_encoded_layers=True)
                sequence_enc_cross = encoded_layers[-1]  # [B L H]

                gate_value = self.gate_[domain_index - len(domain_list)](
                    sequence_emb_cross.view([sequence_emb_cross.size(0), -1]), cross_yn="y", basic_gating=self.basic_gating
                ).unsqueeze(1)  # B x 1 x expert_num
                gate_value_list.append(torch.mean(gate_value, dim=0)[0])
                ...
                fea_2_cross = torch.stack(
                    [self.expert_[i + single_expert_num](sequence_enc_cross).view(sequence_enc_cross.size(0), -1) for i in range(cross_expert_num)], dim=1
                )  # expert_num B T*D
                if self.args.single_detach == "y":
                ...
                gate_value_list.append(torch.mean(gate_value, dim=0)[0])
                fea_1_cross = torch.stack(
                    [
                        self.expert_[i](sequence_emb_cross, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_emb_cross.size(0), -1)
                        for i in range(single_expert_num)
                    ],
                    dim=1,
                )
                fea_2_cross = torch.stack(
                    [
                        self.expert_[i + single_expert_num](sequence_emb_cross, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_emb_cross.size(0), -1)
                        for i in range(cross_expert_num)
                    ],
                    dim=1,
                )  # B x expert_num x (T*hidden_size)
                ...
                pass
            sequence_encoder_output_cross = (
                torch.bmm(gate_value, fea_cross).squeeze(1).view(-1, self.args.max_seq_length, self.args.hidden_size).to(self.device)
            )  # B x T x hidden_size
            ...
            for domain_index in domain_list:
                cross_domain_loss = self.cross_entropy(
                    sequence_encoder_output_cross, item_pos * (type_pos == domain_index), item_neg * (type_pos == domain_index), type_input * (type_pos == domain_index)
                )
            ...
            # final loss
            for domain_index in domain_list:
                cross_domain_loss = self.cross_entropy_single(
                    sequence_encoder_output_cross, item_pos * (type_pos == domain_index), item_neg * (type_pos == domain_index), type_input * (type_pos == domain_index)
                )
            ...
            fea_1_cross = torch.stack(
                [
                    self.expert_[i](sequence_output_shared, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_output_shared.size(0), -1)
                    for i in range(single_expert_num)
                ],
                dim=1,
            )
            fea_2_cross = torch.stack(
                [
                    self.expert_[i + single_expert_num](sequence_output_shared, extended_attention_mask, output_all_encoded_layers=True)[-1].view(
                        sequence_output_shared.size(0), -1
                    )
                    for i in range(cross_expert_num)
                ],
                dim=1,
    ...
    def get_last_emb(self, item_input, type_input, item_pos, item_neg, type_pos, cuda_yn="y"):
        ...
        if self.args.expert_layer == "ffn":
            encoded_layers = self.att_layer(sequence_emb, extended_attention_mask, output_all_encoded_layers=True)
            ...
            gate_value = self.gate_[-1](sequence_emb.view([sequence_emb.size(0), -1]), cross_yn="y", basic_gating=self.basic_gating).unsqueeze(1)  # B x 1 x expert_num
            fea = torch.stack(
                [self.expert_[i](sequence_emb, extended_attention_mask, output_all_encoded_layers=True)[-1].view(sequence_emb.size(0), -1) for i in range(self.expert_num)], dim=1
            )  # # B x expert_num x (T*hidden_size)
