...
from modules import Encoder, Intermediate, LayerNorm
...
class CausalModel(nn.Module):
    ...
    def pretrain_seq(self, item_input, item_pos, item_neg, test_neg, item_answer, cat1_input, cat1_pos, cat1_neg, cat2_input, cat2_pos, cat2_neg, type_input, hierarhical):
        ...
        source_domain = [
            ...
            8,
            9,
        ]  # 6:btv, 7: cdr, 8:tmbr, 9:tmap, 10:11st,11:prod1 // 'books': 5,'clothing_shoes_and_jewelry': 6,'movies_and_tv': 7,'toys_and_games': 8,'sports_and_outdoors': 9

        shaply_values_softmax = self.shaply_layer(
            ...
            item_pos=item_pos,
            item_neg=item_neg,
            cat1_input=cat1_input,
            cat1_pos=cat1_pos,
            cat1_neg=cat1_neg,
            cat2_input=cat2_input,
            cat2_pos=cat2_pos,
            cat2_neg=cat2_neg,
            hierarhical=self.args.hierarhical,
            attention_mask=extended_attention_mask,
            shaply_yn=self.args.shaply_value,
        )
        ...
        encoded_layers = self.item_encoder(sequence_emb, extended_attention_mask, output_all_encoded_layers=True)
        ...
        if self.args.hierarhical == "y":
            ...
            for domain_index in [target_domain] + source_domain:
                loss_contrastive_1 += (
                    self.cross_entropy_shaply(sequence_output, cat2_pos, cat2_neg, type_input, domain_index, hierarchical_level="cat2") * shaply_values_softmax[domain_index]
                )
                loss_contrastive_2 += (
                    self.cross_entropy_shaply(sequence_output_1, cat1_pos, cat1_neg, type_input, domain_index, hierarchical_level="cat1") * shaply_values_softmax[domain_index]
                )
                loss_contrastive_3 += (
                    self.cross_entropy_shaply(sequence_output_2, item_pos, item_neg, type_input, domain_index, hierarchical_level="item") * shaply_values_softmax[domain_index]
                )
            ...
            for domain_index in [target_domain] + source_domain:
                loss_contrastive_3 += (
                    self.cross_entropy_shaply(sequence_output, item_pos, item_neg, type_input, domain_index, hierarchical_level="item") * shaply_values_softmax[domain_index]
                )
    ...
    def get_last_emb(self, item_input, cat1_input, cat2_input, type_input, item_pos, item_neg, hierarhical, cuda_yn="y"):
        ...
        if self.args.hierarhical == "y":
            ...
            cat2_output = sequence_output[
                :,
                -1,
            ]
            cat1_output = sequence_output_1[
                :,
                -1,
            ]
            eos_output = sequence_output_2[
                :,
                -1,
            ]
    ...
    def shaply_layer(
        ...
        if shaply_yn == "y":
            ...
            for k in range(len(shaply_list_tmp)):  # shaply_list
                ...
                sequence_emb_tmp = self.add_position_embedding(
                    item_input * shaply_index_.to(self.device),
                    cat1_input * shaply_index_.to(self.device),
                    cat2_input * shaply_index_.to(self.device),
                    type_input,
                    hierarhical=self.args.hierarhical,
                )

                encoded_layers_tmp = self.item_encoder(sequence_emb_tmp, attention_mask, output_all_encoded_layers=True)
                ...
                if self.args.hierarhical == "y":
                    ...
                    mlm_loss_1_tmp = self.cross_entropy(
                        sequence_output_tmp,
                        cat2_pos * shaply_index_.to(self.device),
                        cat2_neg * shaply_index_.to(self.device),
                        hierarchical_level="cat2",
                        shaply_index=shaply_index_.to(self.device),
                    )
                    mlm_loss_2_tmp = self.cross_entropy(
                        sequence_output_tmp_1,
                        cat1_pos * shaply_index_.to(self.device),
                        cat1_neg * shaply_index_.to(self.device),
                        hierarchical_level="cat1",
                        shaply_index=shaply_index_.to(self.device),
                    )
                    mlm_loss_3_tmp = self.cross_entropy(
                        sequence_output_tmp_2,
                        item_pos * shaply_index_.to(self.device),
                        item_neg * shaply_index_.to(self.device),
                        hierarchical_level="item",
                        shaply_index=shaply_index_.to(self.device),
                    )
            ...
            self.shaply_values_update = self.alpha(self.shaply_values_update.unsqueeze(1).to(self.device)) + self.beta(
                torch.tensor([i for i in shaply_values.values()]).unsqueeze(1).to(self.device)
            )
