...
def main(strd_ym="202303", data_name="amazon"):
    ...
    data_total["cat2_index"] = data_total["cat2"].apply(lambda x: voca_mapping(x, voca[3], timestamp=False))

    data_total.to_pickle(f"data/{data_name}/{data_name}_voca_mapping_{strd_ym}.pkl")

    column_list_ = ["item_index", "type_index", "cat1_index", "cat2_index", "timestamp"]
