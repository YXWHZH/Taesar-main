...
from utils import mrr, ndcg, recall, hr
...
class Trainer(object):
    ...
    def run_epoch(self, i_epoch: int) -> List[List[float]]:
        ...
        total_result = []
        outputs_list = []
        dom_outputs_list = [[] for dom in [0, 1, 2, 3]]
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="validating", leave=False, ncols=70):
                result, dom_result = self.evaluate_batch(batch)
                outputs_list.append(result)
                for dom, dom_result_batch in enumerate(dom_result):
                    dom_outputs_list[dom].append(dom_result_batch)
            ...
            for dom in [0, 1, 2, 3]:
                metric_list, bs = zip(*dom_outputs_list[dom])
                bs = torch.tensor(bs)
                out = defaultdict(list)
                for o in metric_list:
                    for k, v in o.items():
    ...
    def train_batch(self, batch: List[torch.Tensor]) -> List[float]:
        ...
        h = self.model(seq_x, domain_seqs, mask_x, domain_masks, gt_masks)

        # Calculate loss for each domain
        losses = self.model.cal_rec_loss(h, gt, gt_neg, gt_masks)
        total_loss = sum(losses)
        total_loss.backward()

        self.optimizer.step()
        return [ll.item() for ll in losses]
    ...
    def evaluate_batch(self, batch: List[torch.Tensor]) -> List[List[float]]:
        ...
        h = self.model(seq_x, domain_seqs, mask_x, domain_masks_tensor, gt_masks)

        test_item_emb = self.model.ei.weight
        test_logits = torch.matmul(h, test_item_emb.transpose(0, 1))
        ...
        result = {f"{name}@{cutoff}": func(pred, target, cutoff, mean=False) for name, func in [("hr", hr), ("ndcg", ndcg), ("mrr", mrr)] for cutoff in [10, 20, 50, 100]}

        result = {k: v.mean() for k, v in result.items()}

        dom_result = []
        #for dom in [0, 1, 2, 3, 4]: ???
        for dom in [0, 1, 2, 3]:
            mask = gt_masks[dom].squeeze(dim=1)
            dom_test_logits = test_logits[mask]
            dom_item_answer = gt[mask]
            ...
            result = {k: v.mean() for k, v in result.items()}
            dom_result.append((result, dom_test_logits.size(0)))

        return (result, test_logits.size(0)), dom_result
