...
def get_dataset(
    ...
    if args.raw:
        ...
        for seq in tqdm(data_seq, desc="processing", leave=False):
            data_tr.append(process_train(seq[:-2], args.n_doms, args.dom_item_ids, args.len_trim))
        ...
        with open(args.f_data, "wb") as f:
            pickle.dump((data_tr, data_val, data_te, args.dom_item_ids, args.dom_item_nums), f)
    else:
        print("Loading serialized seqs...")
        with open(args.f_data, "rb") as f:
            (data_tr, data_val, data_te, args.dom_item_ids, args.dom_item_nums) = pickle.load(f)
    args.n_item = sum(args.dom_item_nums)
    args.n_user = len(data_tr)
    return (
        TrainDataset(args, data_tr, rng),
        EvalDataset(args, data_val, rng),
        EvalDataset(args, data_te, rng),
    )
