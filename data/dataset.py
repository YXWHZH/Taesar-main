...
class Dataset(torch.utils.data.Dataset):
    ...
    def _get_field_from_config(self):
        ...
        if (self.uid_field is None) ^ (self.iid_field is None):
            raise ValueError("USER_ID_FIELD and ITEM_ID_FIELD need to be set at the same time or not set at the same time.")
    ...
    def _build_feat_name_list(self):
        ...
        """
        feat_name_list = [feat_name for feat_name in ["inter_feat", "user_feat", "item_feat"] if getattr(self, feat_name, None) is not None]
    ...
    def _get_download_url(self, url_file, allow_none=False):
        ...
        if self.dataset_name in dataset2url:
            ...
            raise ValueError(f"Neither [{self.dataset_path}] exists in the device nor [{self.dataset_name}] a known dataset name.")
    ...
    def _init_alias(self):
        ...
        for alias_name_1, alias_1 in self.alias.items():
            for alias_name_2, alias_2 in self.alias.items():
                if alias_name_1 != alias_name_2:
                    ...
                    if len(intersect) > 0:
                        raise ValueError(f"`alias_of_{alias_name_1}` and `alias_of_{alias_name_2}` should not have the same field {list(intersect)}.")
        ...
        for alias_name, alias in self.alias.items():
            ...
            if isin.all() is False:
                raise ValueError(f"`alias_of_{alias_name}` should not contain non-token-like field {list(alias[~isin])}.")
    ...
    def _preload_weight_matrix(self):
        ...
        for preload_id_field, preload_value_field in preload_fields.items():
            ...
            if value_ftype not in {FeatureType.FLOAT, FeatureType.FLOAT_SEQ}:
                self.logger.warning(f"Field [{preload_value_field}] with type [{value_ftype}] is not `float` or `float_seq`, which will not be handled by preload matrix.")
                continue
    ...
    def _filter_nan_user_or_item(self):
        ...
        for field, name in zip([self.uid_field, self.iid_field], ["user", "item"]):
            ...
            if feat is not None:
                ...
                if len(dropped_feat):
                    self.logger.warning(f"In {name}_feat, line {list(dropped_feat + 2)}, {field} do not exist, so they will be removed.")
            ...
            if field is not None:
                ...
                if len(dropped_inter):
                    self.logger.warning(f"In inter_feat, line {list(dropped_inter + 2)}, {field} do not exist, so they will be removed.")
    ...
    def _remove_duplication(self):
        ...
        if self.time_field in self.inter_feat:
            ...
            self.logger.info(f"Records in original dataset have been sorted by value of [{self.time_field}] in ascending order.")
        else:
            self.logger.warning(f"Timestamp field has not been loaded or specified, thus strategy [{keep}] of duplication removal may be meaningless.")
    ...
    def _get_illegal_ids_by_inter_num(self, field, feat, inter_num, inter_interval=None):
        ...
        """
        self.logger.debug(set_color("get_illegal_ids_by_inter_num", "blue") + f": field=[{field}], inter_interval=[{inter_interval}]")
    ...
    def _remap(self, remap_list):
        ...
        tokens_set = sorted(set(tokens), key=int)
        token_id = {tok: idx + 1 for idx, tok in enumerate(tokens_set)}
        token_id["[PAD]"] = 0

        new_ids_list = np.array([token_id[tok] for tok in tokens])
        new_ids_list = np.split(new_ids_list, split_point)
        mp = np.array(["[PAD]"] + tokens_set)
        ...
        for feat_name, unused_fields in unused_col.items():
            ...
            for field in unused_fields:
                if field not in feat:
                    self.logger.warning(f"Field [{field}] is not in [{feat_name}_feat], which can not be set in `unused_col`.")
                    continue
    ...
    def build(self):
        ...
        if split_mode == "RS":
            ...
            datasets = self.time_based_split(ratios=split_args["TS"], group_by=self.uid_field)
        else:
    ...
    def _history_matrix(self, row, value_field=None, max_history_len=None):
        ...
        if col_num > max_col_num * 0.2:
            self.logger.warning(f"Max value of {row}'s history interaction records has reached {col_num / max_col_num * 100}% of the total.")
